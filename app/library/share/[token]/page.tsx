import { getPortalLibrarySnapshotByShareToken } from "@/lib/portal-library"
...
export default async function PublicLibraryPage({ params }: { params: { token: string } }) {
  const { client, assets, colors, typefaces } = await getPortalLibrarySnapshotByShareToken(params.token)
    ...
    <>
      ...
      <div className="hero-pad" style={{
        ...
        <div>
          ...
          <div style={{
            ...
            Logo files, colors, typography, and deliverables
          </div>
        </div>
        {assets.length > 0 && (
      ...
      {isEmpty ? (
        <div style={{ padding: "80px 48px", textAlign: "center", maxWidth: 480, margin: "0 auto" }}>
          ...
          <div style={{ fontFamily: "var(--font-sans)", fontSize: 20, opacity: 0.72, marginBottom: 14, letterSpacing: "-0.01em" }}>
            Nothing here yet.
          </div>
          <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", opacity: 0.65, lineHeight: 1.8 }}>
            Brand assets will appear here once they&apos;re ready.
          </div>
        </div>
      ) : (
                  ...
                  {typefaces.map((tf: any, i: number) => (
                    <div key={tf.id} style={{
                      ...
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", color: "var(--ink)", opacity: 0.72, marginBottom: 3 }}>
                          {tf.name} — {tf.weight}
                        </div>
                        ...
                        {tf.file_url && (
                          <a href={tf.file_url} style={{ fontFamily: "var(--font-mono)", fontSize: "var(--text-eyebrow)", color: "var(--ink)", opacity: 0.65, letterSpacing: "0.1em", textTransform: "uppercase", textDecoration: "none" }}>
                            ↓ Download
                          </a>
                        )}
                      </div>
...
function FileRow({ asset }: { asset: any }) {
  ...
  return (
    <a href={asset.file_url} style={{
      ...
      <div style={{ fontFamily: "var(--font-mono)", fontSize: "var(--text-eyebrow)", color: "var(--ink)", opacity: 0.65, letterSpacing: "0.1em", textTransform: "uppercase" }}>
        ↓ Download
      </div>
    </a>
  )
