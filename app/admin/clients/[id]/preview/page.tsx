...
import { getPortalLibrarySnapshotByClientId } from "@/lib/portal-library"
...
const statusLabels: Record<string, string> = {
  ...
  awaiting_client: "Awaiting Feedback",
  awaiting_payment: "Files Delivered — Awaiting Payment",
...
function heroContent(status: string, firstName: string, hasProject: boolean, isComplete: boolean) {
  if (!hasProject) {
    return { eyebrow: null, greeting: `Welcome, ${firstName}`, subline: "Your portal will come to life once your project begins." }
  }
  if (status === "awaiting_approval") {
    return { eyebrow: { label: "Awaiting your review", color: "var(--amber)" }, greeting: `${firstName}, your feedback is needed`, subline: "A deliverable is ready for your review." }
  }
  if (isComplete) {
    return { eyebrow: { label: "Complete", color: "rgba(15,15,14,0.34)" }, greeting: `Hello, ${firstName}`, subline: "Your project is complete. Final files and materials are available in your library." }
  }
  return {
...
type NextStep = { label: string; body: string; href: string; accent: string; tag: string }

/* ─── Page ─── */

export default async function AdminClientPreviewPage({ params }: { params: { id: string } }) {
  const clientId = params.id

  /* ─── Fetch all client data using admin client (bypasses RLS) ─── */
  ...
  if (!client) {
    return (
      <main style={{ maxWidth: 980, margin: "0 auto", padding: 40, textAlign: "center" }}>
        <p style={{ fontFamily: "var(--font-sans)", color: "var(--ink)", opacity: 0.5 }}>Client not found.</p>
      </main>
    )
  }
  ...
  const portalLibrary = await getPortalLibrarySnapshotByClientId(clientId)

  const [projectsRes, invoicesRes, proposalsRes] = await Promise.all([
    supabaseAdmin.from("projects").select("*, deliverables(*)").eq("client_id", clientId).order("created_at", { ascending: false }),
    supabaseAdmin.from("invoices").select("*").eq("client_id", clientId).order("created_at", { ascending: false }),
    supabaseAdmin.from("proposals").select("*, proposal_items(*)").eq("client_id", clientId).eq("status", "accepted").order("created_at", { ascending: false }).limit(1),
  ])
    ...
    <>
      ...
      <main style={{ maxWidth: 980, margin: "0 auto", padding: "clamp(24px, 4vw, 40px)" }}>
        ...
        <section style={{
          ...
          <div style={{ maxWidth: 580 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 18 }}>
              {hero.eyebrow && (
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: hero.eyebrow.color, flexShrink: 0 }} />
              )}
              <span style={{
                fontFamily: "var(--font-mono)",
        ...
        {activeProjects.length > 0 ? (
          ...
          <section style={{ marginBottom: 40, padding: "48px 0", textAlign: "center" }}>
            <div style={{
              fontFamily: "var(--font-serif)",
              fontStyle: "italic",
              fontSize: 20,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {approvedScope.map((item: any) => (
                  <div key={item.id} style={{
                    ...
                    gap: 16, padding: "12px 0", borderBottom: "0.5px solid rgba(15,15,14,0.06)",
                  }}>
                    <div>
                      <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", color: "var(--ink)", opacity: 0.76, letterSpacing: "-0.01em" }}>
                        {item.name}
                      </div>
                      {item.description && (
                        <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", color: "var(--ink)", opacity: 0.38, marginTop: 2 }}>
                          {item.description}
                        </div>
                      )}
                    </div>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--text-eyebrow)", letterSpacing: "0.06em", color: "var(--ink)", opacity: 0.32, flexShrink: 0 }}>
              ...
              {laterPhase.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <div style={{
                    ...
                    color: "var(--ink)", opacity: 0.28, marginBottom: 10,
                  }}>
                  ...
                  {laterPhase.map((item: any) => (
                    <div key={item.id} style={{
                      display: "flex", justifyContent: "space-between", alignItems: "baseline",
                      gap: 16, padding: "10px 0", borderBottom: "0.5px solid rgba(15,15,14,0.04)",
                    }}>
                      <span style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", color: "var(--ink)", opacity: 0.46 }}>
                        {item.name}
                      </span>
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--text-eyebrow)", letterSpacing: "0.06em", color: "var(--ink)", opacity: 0.22, flexShrink: 0 }}>
                        {formatCurrency(item.price)}
                      </span>
                    </div>
                  ))}
                    ...
                    >
                      <div>
                        <div style={{
                          ...
                          letterSpacing: "0.14em", textTransform: "uppercase",
                          color: step.accent, opacity: 0.88, marginBottom: 6,
                        }}>
                          {step.tag}
                        </div>
            ...
            <div style={sectionBodyWrapStyle}>
              {paidTotal > 0 ? (
                <>
                  ...
                  {totalContract > 0 && (
                    <div style={{
                      width: "100%", height: 2, background: "rgba(15,15,14,0.06)",
                      margin: "16px 0", position: "relative", overflow: "hidden",
                    }}>
                      <div style={{
                        position: "absolute", top: 0, left: 0, bottom: 0,
                        width: `${paidPct}%`, background: "var(--sage)", opacity: 0.7,
                      }} />
                    </div>
                  )}
              ...
              <div className="portal-button-soft" style={inlineButtonStyle}>
                View Billing
              </div>
            </div>
          </section>
