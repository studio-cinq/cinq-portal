...
export async function POST(req: Request) {
  try {
    const { foundationId } = await req.json()
    ...
    await trySendEmail({
      ...
      subject: `Brand Foundations viewed — ${client.name}`,
      html: `
        <div style="font-family:Georgia,serif;max-width:500px;margin:40px auto;color:#1C1916">
          ...
          <h2 style="font-weight:normal;font-size:20px;margin:8px 0 24px">${client.contact_name} is viewing their Brand Foundations</h2>
    ...
    console.error("[notify/foundations-viewed]", err)
