...
/**
 * Renders a single invoice as a one-page PDF and returns the raw bytes.
 *
 * Called by the /api/pdf/invoice/[id] GET route (client download) AND by the
 * outbound-invoice email flow when a client has `attach_pdf_to_emails=true`
 * (their AP system ingests PDF attachments rather than following links).
 */
...
export async function generateInvoicePdf(invoiceId: string): Promise<{ buffer: Buffer; filename: string } | null> {
  ...
  // that shouldn't appear on the downloaded PDF.
  ...
  const details = [
    ...
    { label: "DATE ISSUED", value: new Date(invoice.last_sent_at ?? invoice.created_at).toLocaleDateString("en-US", { timeZone: "America/New_York", month: "long", day: "numeric", year: "numeric" }) },
  ...
  // Venmo (per-invoice opt-in; hidden unless the handle env var is set)
  const venmoHandle = (process.env.NEXT_PUBLIC_VENMO_HANDLE ?? "").replace(/^@/, "")
  if (paymentMethods.includes("venmo") && venmoHandle && !isPaid) {
    y += 24
    doc.setDrawColor(200, 196, 190)
    doc.setLineWidth(0.3)
    doc.line(marginL, y, W - marginR, y)
    ...
    for (const row of venmoRows) {
      ...
      setColor(doc, INK, 0.75)
      if (row.label === "HANDLE") {
        doc.textWithLink(row.value, marginL + 120, y, { url: `https://venmo.com/u/${venmoHandle}` })
      } else {
        doc.text(row.value, marginL + 120, y)
      }
      y += 16
