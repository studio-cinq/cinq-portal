...
/**
 * Renders a Client-level Statement of Account as a one-page PDF and returns
 * the raw bytes. Same document as the /api/pdf/statement/[clientId] download,
 * used as an email attachment on batched client reminders when the client
 * has `attach_pdf_to_emails=true` (their AP system ingests PDF attachments).
 */
...
export async function generateStatementPdf(clientId: string): Promise<{ buffer: Buffer; filename: string } | null> {
  ...
  // Optional ACH block if configured
